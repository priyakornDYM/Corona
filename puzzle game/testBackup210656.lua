...
local lockGemMutiChk = false --- chk slide lock for seconde muit add on
local gemChkMark = {}
...
function slideGemAfter(self,event)    
   -- print(self.chkFtPosit)
    if (self.chkFtPosit == "x") then -- -- -- -- -- slide X           
        if ( lineY ~=nil) then
            lineY:removeSelf()         
       ...
       -- gemsTable[][].j :: gemsTable[][].i == point self // gemsTable[self.i][self.j] == data self VIP                         
        if (gemsTable[self.i][self.j].x <= 20 or gemsTable[self.i][self.j].x >= 620) then     --  jump end dont move                  
            ...
            for posX = 1, gemX, 1 do                 
                ...
                --print("  sdsdsd "..gemsTable[posX][self.j].x)
...
local function chkSlideLock(self)         
      local cnt = 0   
      ...
      for  i = 1, gemX, 1 do
          ...
          for  j = 1, gemY, 1 do              
              if(gemsTable[i][j].isMarkedToDestroy == false ) then                  
                   -- print("c ooooo "..gemsTable[i][j].i, gemsTable[i][j].j)
              else    
                  cnt = cnt+1
                  print(cnt,gemsTable[i][j].y)                
  --                gemChkMark[cnt].x = gemsTable[i][j].x
--                  gemChkMark[cnt].y = gemsTable[i][j].y
--                  gemChkMark[cnt].i = gemsTable[i][j].i
--                  gemChkMark[cnt].j = gemsTable[i][j].j                  
                  
             --     gemChkMark[cnt] = gemsTable[i][j].x..","..gemsTable[i][j].y..","..gemsTable[i][j].i..","..gemsTable[i][j].j
               ---  print("postion "..gemsTable[i][j].x,gemsTable[i][j].y)
                 -- print("c xxxxx "..gemsTable[i][j].i, gemsTable[i][j].j)              
              end            
            --  print("test i".. gemsTable[i][j].i.." j".. gemsTable[i][j].j.."  "  ..gemsTable[i][j].colorR )--
          end
       end    
       
--       if (gemChkMark~=nil) then       
--            for  i = 1, #gemChkMark do
--                print(gemChkMark[i])
--            end            
--        end
   ...
   elseif self.isFocus then       
    ...
    --          print( "posX == self.markX   moveY")
         ...
         --          print("sssssssss sss sss s ss ")
           ...
           if(lockGemMutiChk == true) then
                slideGemAfter(self,event)  --- old     
           else
